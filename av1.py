...
def save_faiss_index(index, folder_path):
    ...
    faiss.write_index(index, os.path.join(folder_path, "faiss_index"))

# --- Load FAISS Index ---
def load_faiss_index(folder_path):
    index_file = os.path.join(folder_path, "faiss_index")
...
def main():
    ...
    # Folder path to store FAISS index
    faiss_folder = "./faiss_data"

    # Check if FAISS index exists
    index = load_faiss_index(faiss_folder)
    if index:
        st.success("FAISS Index Loaded from Storage!")
    ...
    if uploaded_file:
        ...
        if st.button("Generate Embeddings"):
            ...
            if embeddings:
                ...
                save_faiss_index(index, faiss_folder)  # Save the FAISS index
